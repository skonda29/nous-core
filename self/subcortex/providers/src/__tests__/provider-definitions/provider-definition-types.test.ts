...
type _ProviderVendorKeyIsExact = Expect<
  Equal<ProviderVendorKey, 'anthropic' | 'codex-cli' | 'deepinfra' | 'github-copilot-cli' | 'groq' | 'llama-cpp' | 'moonshot' | 'openai' | 'ollama' | 'openrouter'>
>;
type _BootstrapProviderKeyIsExact = Expect<
  Equal<BootstrapProviderKey, 'anthropic' | 'codex-cli' | 'deepinfra' | 'github-copilot-cli' | 'groq' | 'llama-cpp' | 'moonshot' | 'openai' | 'ollama' | 'openrouter'>
>;
...
describe('provider definition type derivation', () => {
  it('derives runtime roster keys from the production tuple', () => {
    ...
    expect(keys.sort()).toEqual(['anthropic', 'codex-cli', 'deepinfra', 'github-copilot-cli', 'groq', 'llama-cpp', 'moonshot', 'ollama', 'openai', 'openrouter']);
  });
