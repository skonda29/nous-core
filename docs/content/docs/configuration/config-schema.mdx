...
- **vendor** (optional): Certified provider vendor key for adapter selection. Known vendor values include `anthropic`, `openai`, and `ollama`; see [Provider Adapters](/docs/development/provider-adapters) for how vendor keys map to provider leaves. Unknown values fall back through the adapter resolver rather than becoming certified vendors.
