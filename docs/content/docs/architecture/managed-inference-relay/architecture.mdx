...
description: Problem statement, architecture objective, and core concepts for managed inference relay
---
...
Nue selects the backend route based on requested capability, tenant policy, plan limits, model quality, cost efficiency, latency, regional availability, provider health, quota, and fallback policy.
...
Tenant auth is authentication from the client to NueOS Cloud.
