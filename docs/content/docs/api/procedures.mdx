...
- `preferences.getModelSelection` / `preferences.setModelSelection` — Persist the separate Principal and System model selections that Nous restores on startup. `setModelSelection` accepts cloud specs and `ollama:*` specs, then updates the in-memory provider config and model role assignment immediately so model changes take effect without a restart. Invalid model specs are rejected gracefully without corrupting provider state.
